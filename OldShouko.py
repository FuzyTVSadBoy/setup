...
Plr.OnTeleport:Connect(function(teleportState)
    if teleportState == Enum.TeleportState.Started or teleportState == Enum.TeleportState.InProgress then
        isTp = true
        teleportFailedUntil = 0
        log("TELEPORT")
    elseif teleportState == Enum.TeleportState.Failed then
        isTp = false
        teleportFailedUntil = os.time() + 15
        log("TELEPORT_FAILED")
    end
end)
...
task.spawn(function()
    while true do
        ...
        if hasError then
            log("ERROR")
        elseif teleportFailedUntil > os.time() then
            log("TELEPORT_FAILED")
        elseif isTp then
            log("TELEPORT")
        else
...
class RobloxManager:
    @staticmethod
    def _discover_package_pids(package_name):
        pids = []
        seen = set()
        ...
        for cmd in (["/system/bin/pidof", package_name], ["pidof", package_name]):
            try:
        ...
        try:
            suffix = package_name[-15:].lower()
            for proc in process_iter(['name', 'pid']):
                proc_name = (proc.info.get('name') or "").lower()
                if proc_name == suffix:
                    add_pid(proc.info.get('pid'))
        except (NoSuchProcess, AccessDenied, ZombieProcess):
            pass
        except Exception:
            pass
    ...
    @staticmethod
    def _kill_package_pids(package_name, pids):
        killed = []

        for pid in pids:
            for cmd in (["/system/bin/kill", "-9", pid], ["kill", "-9", pid]):
                try:
                    result = subprocess.run(cmd, capture_output=True, text=True)
                    if result.returncode == 0:
                        killed.append(pid)
        ...
        if killed:
            print(f"\033[1;32m[ Shouko.dev ] - Silently killed PID(s) {', '.join(killed)} for {package_name}\033[0m")
            return True
        return False
    ...
    def kill_roblox_processes():
        ...
        killed_any = False
        for package_name in packages:
            pids = RobloxManager._discover_package_pids(package_name)
            if RobloxManager._kill_package_pids(package_name, pids):
                killed_any = True
        if not killed_any:
    ...
    def kill_roblox_process(package_name):
        ...
        try:
            ...
            if not RobloxManager._kill_package_pids(package_name, pids):
                # Bắn lệnh kill -9 tàn bạo nhất vào thẳng nhân Linux
                print(f"\033[1;33m[ Shouko.dev ] - No live PID found for {package_name}; skipped force-stop to keep Launcher stable.\033[0m")
...
|___/_| |_|\___/ \__,_|_|\_\___(_)__,_|\___| \_/  
    ...
    @staticmethod
    def stop_live():
        with UIManager.live_lock:
            if UIManager.live is not None:
                try:
                    UIManager.live.stop()
                except Exception:
                    pass
    ...
    @staticmethod
    def _short_package_name(package_name, max_length):
        package_name = str(package_name or "")
        if max_length <= 2:
            return package_name[-max_length:]
        if len(package_name) <= max_length:
            return package_name
        return ".." + package_name[-(max_length - 2):]

    @staticmethod
    def _status_style(status_text):
        plain = UIManager._strip_ansi(status_text)
        if any(token in plain for token in ("Kick / Crash", "Rejoining", "Stuck", "No Signal > 180s", "Executor didn't load", "Error checking executor")):
            return "bold red"
        if any(token in plain for token in ("Teleporting",)):
            return "bold magenta"
        if any(token in plain for token in ("Booting", "Waiting signal", "Checking executor", "Opening Roblox", "Joining Roblox", "Teleport Failed")):
            return "bold yellow"
        if any(token in plain for token in ("Launching", "Rejoin")):
            return "bold cyan"
        return "bold green"
    ...
    def _build_status_renderable():
        ...
        term_width = max(32, UIManager.console.size.width)
        no_width = 4
        padding_budget = 6
        available_width = max(26, term_width - padding_budget)
        remaining_width = max(22, available_width - no_width)
        account_width = min(22, max(10, remaining_width // 3))
        status_width = max(12, remaining_width - account_width)
        package_width = min(10, max(6, account_width - 4))

        info_line = Text()
        info_line.append("CPU ", style="bold cyan")
        info_line.append(f"{cpu_usage:.1f}%", style="white")
        info_line.append(" | RAM ", style="bold cyan")
        info_line.append(f"{ram:.1f}%", style="white")
        info_line.append(" | Mode ", style="bold cyan")
        info_line.append(method, style="white")
        ...
        table.add_column("No", justify="right", width=no_width, no_wrap=True, style="bold cyan")
        table.add_column("Account", width=account_width, no_wrap=True, overflow="ellipsis")
        table.add_column("Status", width=status_width, no_wrap=True, overflow="ellipsis")

        for index, (package, info) in enumerate(sorted(globals().get("package_statuses", {}).items()), start=1):
            username = str(info.get("Username", "Unknown"))
            if username != "Unknown":
                username = "******" + username[6:] if len(username) > 6 else "******"

            show_package = account_width >= 16
            short_package = UIManager._short_package_name(package, package_width) if show_package else ""
            username_width = max(4, account_width - (len(short_package) + 1 if short_package else 0))
            account_name = UIManager._truncate_text(username, username_width)

            account_text = Text(account_name, style="bold cyan")
            if short_package:
                if account_name:
                    account_text.append(" ")
                account_text.append(short_package, style="dim white")

            plain_status = UIManager._strip_ansi(info.get("Status", "Unknown"))
            status_text = Text(
                UIManager._truncate_text(plain_status, status_width),
                style=UIManager._status_style(plain_status)
            )

            table.add_row(str(index), account_text, status_text)

        return Group(info_line, table)
    ...
    def update_status_table():
        ...
        renderable = UIManager._build_status_renderable()
        with UIManager.live_lock:
            if UIManager.live is None:
                UIManager.live = Live(
                    renderable,
                ...
                UIManager.live.start()
            UIManager.live.update(renderable, refresh=True)
...
class Runner:
    ...
    @classmethod
    def _focus_shift_score(cls, package_name):
        status_text = UIManager._strip_ansi(
            globals().get("package_statuses", {}).get(package_name, {}).get("Status", "")
        )

        if any(token in status_text for token in ("Script Connected", "Joined")):
            return 3
        if any(token in status_text for token in ("Waiting signal", "Teleporting", "Booting", "Opening Roblox", "Joining Roblox")):
            return 2
        if any(token in status_text for token in ("Kick / Crash", "Rejoining", "Stuck", "No Signal > 180s", "Executor didn't load", "Error checking executor")):
            return -1
        return 1 if status_text else 0

    @classmethod
    def focus_shift_to_healthy_package(cls, current_package, server_links):
        candidate = None
        candidate_score = -1

        for other_pkg, _ in server_links:
            if other_pkg == current_package:
                continue

            score = cls._focus_shift_score(other_pkg)
            if score > candidate_score:
                candidate = other_pkg
                candidate_score = score

        if not candidate or candidate_score < 1:
            return False
        ...
        try:
            subprocess.run(
                [
                    '/system/bin/am', 'start',
                    '-n', f'{candidate}/com.roblox.client.startup.ActivitySplash'
                ],
            ...
            try:
                ...
                for pkg, _ in server_links:
                    ...
                    if rejoin:
                        ...
                        print("\033[1;35m[ Shouko.dev ] - Focus shifting sang tab khỏe trước khi kill...\033[0m")
                        Runner.focus_shift_to_healthy_package(pkg, server_links)
...
def main():
    ...
    if globals().get("check_exec_enable") == "1":
        globals()["lua_script_template"] = DEFAULT_HEARTBEAT_LUA
        try:
            os.makedirs("Shouko.dev", exist_ok=True)
            with open("Shouko.dev/checkui.lua", "w", encoding="utf-8") as f:
                f.write(DEFAULT_HEARTBEAT_LUA)
            FileManager.save_config()
        except Exception as e:
            print(f"\033[1;31m[ Script ] Error refreshing heartbeat script: {e}\033[0m")
...
"""
                ...
                if globals()["lua_script_template"]:
                    with open(cfg_path, "w", encoding="utf-8") as f: f.write(globals()["lua_script_template"])
