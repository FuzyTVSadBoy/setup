...
def main():
    ...
    LUA_NEW = r"""local RunService=game:GetService("RunService");local CoreGui=game:GetService("CoreGui");local GuiService=game:GetService("GuiService");local TeleportService=game:GetService("TeleportService");local Players=game:GetService("Players");local LP=Players.LocalPlayer;local FILE="heartbeat_"..tostring(LP.UserId)..".txt";local isTp=false;local function log(s)pcall(function()if writefile then writefile(FILE,s.."|"..tostring(os.time()))end end)end;local function isErr()local p=CoreGui:FindFirstChild("RobloxPromptGui");if p then local o=p:FindFirstChild("promptOverlay")if o and o.Visible then return true end end;local s,m=pcall(function()return GuiService:GetErrorMessage()end)if s and m and m~=""then return true end;if not LP or not LP.Parent then return true end;return false end;task.spawn(function()log("ALIVE")while true do task.wait(2)if isTp then else if isErr()then log("ERROR")else log("ALIVE")end end end end);LP.OnTeleport:Connect(function(s)if s==Enum.TeleportState.Started then isTp=true;log("TELEPORT")end end);TeleportService.TeleportInitFailed:Connect(function()isTp=false;log("ALIVE")end);Players.PlayerRemoving:Connect(function(p)if p==LP then log("ERROR")end end);GuiService.ErrorMessageChanged:Connect(function()if GuiService:GetErrorMessage()~=""then log("ERROR")end end)"""
    ...
    while True:
        ...
        if setup_type == "2":
            ...
            try:
                LUA_SRC = r"""local RunService=game:GetService("RunService");local CoreGui=game:GetService("CoreGui");local GuiService=game:GetService("GuiService");local TeleportService=game:GetService("TeleportService");local Players=game:GetService("Players");local LP=Players.LocalPlayer;local FILE="heartbeat_"..tostring(LP.UserId)..".txt";local isTp=false;local function log(s)pcall(function()if writefile then writefile(FILE,s.."|"..tostring(os.time()))end end)end;local function isErr()local p=CoreGui:FindFirstChild("RobloxPromptGui");if p then local o=p:FindFirstChild("promptOverlay")if o and o.Visible then return true end end;local s,m=pcall(function()return GuiService:GetErrorMessage()end)if s and m and m~=""then return true end;if not LP or not LP.Parent then return true end;return false end;task.spawn(function()log("ALIVE")while true do task.wait(2)if isTp then else if isErr()then log("ERROR")else log("ALIVE")end end end end);LP.OnTeleport:Connect(function(s)if s==Enum.TeleportState.Started then isTp=true;log("TELEPORT")end end);TeleportService.TeleportInitFailed:Connect(function()isTp=false;log("ALIVE")end);Players.PlayerRemoving:Connect(function(p)if p==LP then log("ERROR")end end);GuiService.ErrorMessageChanged:Connect(function()if GuiService:GetErrorMessage()~=""then log("ERROR")end end)"""
