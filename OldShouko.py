...
class Runner:
    ...
    def monitor_presence(cls, server_links, stop_event):
        print("\033[1;36m[ Runner ] - Smart Monitor V16 (Kick/Tp/LowCPU)...\033[0m")
        low_cpu_start = {}
                ...
                for pkg, _ in server_links:
                    ...
                    if not rejoin:
                        ...
                        if now - last_hb < cls.HEARTBEAT_TIMEOUT and last_hb > 0:
                            # Reset cờ teleport cũ
                            if status_lua != "TELEPORT" and uid in cls.teleport_start:
                                del cls.teleport_start[uid]

                            # Xử lý các trạng thái từ Script
                            if status_lua == "KICK":
                                st = "\033[1;31mKICKED by Game\033[0m"
                                rejoin = True
                            
                            elif status_lua == "SHUTDOWN":
                                st = "\033[1;31mGame Closed\033[0m"
                                rejoin = True
                        ...
                        # 3. NO SIGNAL CHECK
                            ...
                            # Nếu đang Teleport mà mất tín hiệu (do chuyển server)
