...
class Runner:
    BOOT_GRACE = 300       # 5 phút đầu
    HEARTBEAT_TIMEOUT = 15 # 10s không có file -> Nghi ngờ
    TELEPORT_WAIT_MAX = 60 # Chờ tối đa 60s khi nghi ngờ teleport
    
    launch_times = {}
    proc_cache = {}
    path_cache = {}
    teleport_start = {}    # Lưu thời điểm bắt đầu nghi ngờ teleport
    ...
    @classmethod
    def get_last_timestamp(cls, user_id):
        """Đọc timestamp từ file heartbeat và XÓA FILE ngay sau khi đọc"""
        filename = f"heartbeat_{user_id}.txt"
        timestamp = 0
        file_path_found = None

        # 1. Check cache path trước
        ...
        if file_path_found:
            try:
                with open(file_path_found, 'r') as f:
                    ...
                    if content:
                        timestamp = int(float(content))
                
                # --- QUAN TRỌNG: XÓA FILE SAU KHI ĐỌC ---
                os.remove(file_path_found)
                # ----------------------------------------
            except: 
                pass
    ...
    def launch_package_sequentially(cls, server_links):
        ...
        for pkg, link in server_links:
            ...
            print(f"\033[1;32m[ Runner ] Launching {pkg}...\033[0m")
    ...
    def monitor_presence(cls, server_links, stop_event):
        print("\033[1;36m[ Runner ] - Monitor (Low CPU Rejoin + High CPU Teleport)...\033[0m")
        high_cpu_start = {} # Đếm thời gian CPU cao
        ...
        while not stop_event.is_set():
            try:
                ...
                for pkg, _ in server_links:
                    ...
                    last_hb = cls.get_last_timestamp(uid)
                    ...
                    # === TRƯỜNG HỢP 1: CÓ HEARTBEAT MỚI ===
                    if now - last_hb < cls.HEARTBEAT_TIMEOUT:
                        # Reset mọi bộ đếm lỗi
                        if uid in cls.teleport_start: del cls.teleport_start[uid]
                        if uid in high_cpu_start: del high_cpu_start[uid]
                        if uid in low_cpu_start: del low_cpu_start[uid]
                        
                        st = f"\033[1;32mScript Connected (Ping: {now-last_hb:.1f}s) | CPU: {pkg_cpu:.1f}%\033[0m"

                    # === TRƯỜNG HỢP 2: MẤT HEARTBEAT ===
                    else:
                        # -------------------------------------------------
                        # LOGIC 1: CHECK TREO MÁY (LOW CPU) - ƯU TIÊN CAO
                        # Chạy cả lúc Booting lẫn lúc mất kết nối
                        # -------------------------------------------------
                        if (now - lt_launch > 20) and (pkg_cpu < 20.0): # CPU < 20%
                            if uid not in low_cpu_start: 
                                low_cpu_start[uid] = now
                            elif now - low_cpu_start[uid] > 15: # Quá 15s
                                st = f"\033[1;31mFROZEN/CRASH ({pkg_cpu:.1f}% > 15s)\033[0m"
                                rejoin = True # --> REJOIN NGAY
                        else:
                            if uid in low_cpu_start: del low_cpu_start[uid]

                        # Nếu đã quyết định Rejoin do Low CPU thì bỏ qua các check khác
                        if rejoin: pass 
                        
                        # -------------------------------------------------
                        # LOGIC 2: CHECK BOOTING (GRACE PERIOD)
                        # -------------------------------------------------
                        ...
                        # -------------------------------------------------
                        # LOGIC 3: CHECK TELEPORT (HIGH CPU)
                        # -------------------------------------------------
                        else:
                            is_teleporting = False
                            
                            # Nếu chưa xác nhận đang teleport
                            if uid not in cls.teleport_start:
                                # Kiểm tra CPU cao (>30%) liên tục 10s
                                if pkg_cpu > 30.0:
                                    if uid not in high_cpu_start: high_cpu_start[uid] = now
                                    elif now - high_cpu_start[uid] > 10:
                                        cls.teleport_start[uid] = now # Xác nhận Teleport
                                        is_teleporting = True
                                else:
                                    if uid in high_cpu_start: del high_cpu_start[uid]
                            else:
                            ...
                            if is_teleporting:
                                wait_time = int(now - cls.teleport_start[uid])
                                if wait_time < cls.TELEPORT_WAIT_MAX:
                                    st = f"\033[1;35mTeleporting? (Wait {cls.TELEPORT_WAIT_MAX - wait_time}s)\033[0m"
                                else:
                                    st = "\033[1;31mTeleport Stuck (Timeout)\033[0m"
                                    rejoin = True
                            else:
                                st = "\033[1;31mNO SIGNAL (Dead)\033[0m"
                                rejoin = True
...
def main():
    ...
    LUA_NEW = r"""local Plr = game:GetService("Players").LocalPlayer
local FILE = "heartbeat_"..tostring(Plr.UserId)..".txt"
task.spawn(function()
    while true do
        if writefile then writefile(FILE, tostring(os.time())) end
        task.wait(2) -- Ghi mỗi 2s (nhanh hơn 5s của Python để trừ hao)
    end
end)"""
    ...
    while True:
        ...
        if setup_type == "2":
            ...
            try:
                LUA_SRC = r"""local Plr = game:GetService("Players").LocalPlayer
local FILE = "heartbeat_"..tostring(Plr.UserId)..".txt"
task.spawn(function()
    while true do
        if writefile then writefile(FILE, tostring(os.time())) end
