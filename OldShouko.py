...
class Runner:
    ...
    HEARTBEAT_TIMEOUT = 10 # 10s không có file -> Nghi ngờ
    ...
    def get_last_timestamp(cls, user_id):
        """Đọc timestamp từ file heartbeat đơn giản"""
        ...
        if user_id in cls.path_cache:
            ...
            if os.path.exists(path):
                try: return int(float(open(path, 'r').read().strip()))
                except: pass
    ...
    def monitor_presence(cls, server_links, stop_event):
        print("\033[1;36m[ Runner ] - Simple Heartbeat Monitor (CPU Check)...\033[0m")
        ...
        while not stop_event.is_set():
            try:
                ...
                for pkg, _ in server_links:
                    ...
                    # === TRƯỜNG HỢP 2: MẤT HEARTBEAT (HOẶC CHƯA CÓ) ===
                    else:
                        # A. Đang Booting (Grace Period)
                        if now - lt_launch < cls.BOOT_GRACE:
                            rem = int(cls.BOOT_GRACE - (now - lt_launch))
                            st = f"\033[1;33mBooting ({rem}s) | CPU: {pkg_cpu:.1f}%\033[0m"
                        
                        # B. Hết Boot Grace -> Mất kết nối thật sự
                        else:
                            # --- LOGIC SUY LUẬN TELEPORT DỰA TRÊN CPU ---
                            is_teleporting = False
                            
                            # Nếu chưa xác nhận đang teleport
                            if uid not in cls.teleport_start:
                                # Kiểm tra CPU cao (>30%) liên tục
                                if pkg_cpu > 30.0:
                                    if uid not in high_cpu_start: high_cpu_start[uid] = now
                                    elif now - high_cpu_start[uid] > 10: # CPU cao > 10s
                                        # Xác nhận: Có thể đang Teleport
                                        cls.teleport_start[uid] = now
                                        is_teleporting = True
                                else:
                                    if uid in high_cpu_start: del high_cpu_start[uid]
                            else:
                            ...
                            if is_teleporting:
                                ...
                                if wait_time < cls.TELEPORT_WAIT_MAX:
                                    st = f"\033[1;35mTeleporting? (CPU High) - Wait {cls.TELEPORT_WAIT_MAX - wait_time}s\033[0m"
                                else:
                                    st = "\033[1;31mTeleport Timeout (Stuck)\033[0m"
                                    rejoin = True
                            else:
                                st = "\033[1;31mNO SIGNAL (Low CPU / Crash)\033[0m"
                                rejoin = True
            ...
            time.sleep(5) # Quét mỗi 5s theo yêu cầu
...
task.spawn(function()
    while true do
        ...
        task.wait(3) -- Ghi mỗi 2s (nhanh hơn 5s của Python để trừ hao)
    end
end)"""
