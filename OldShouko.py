...
class Runner:
    ...
    def launch_package_sequentially(cls, server_links):
        ...
        for pkg, link in server_links:
            ...
            with status_lock:
                globals()["package_statuses"][pkg]["Status"] = "\033[1;33mBooting (Grace Period)...\033[0m"
            time.sleep(15)
                ...
                for pkg, _ in server_links:
                    ...
                    if not hb:
                        ...
                        if now - lt < cls.BOOT_GRACE:
                            ...
                            if (now - lt > 20) and cpu < 3.0:
                                if uid not in low_cpu_start: low_cpu_start[uid] = now
                                elif now - low_cpu_start[uid] > 40:
                                    st = "\033[1;31mFROZEN LOGO (Low CPU)\033[0m"; rejoin = True
                            else:
