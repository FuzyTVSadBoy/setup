...
def find_all_workspaces():
    """Quét toàn bộ bộ nhớ để tìm thư mục Workspace tiềm năng"""
    possible_paths = set()
    
    # 1. Thêm từ danh sách executors có sẵn
    for base_path in executors.values():
        possible_paths.add(os.path.join(base_path, "Workspace"))
        possible_paths.add(os.path.join(base_path, "workspace"))
    
    # 2. Thêm các đường dẫn phổ biến khác (cho Cloud Phone / Clone)
    common_roots = [
        "/storage/emulated/0",
        "/sdcard",
    ...
    for root in common_roots:
        if os.path.exists(root):
            try:
                # Quét 2 tầng thư mục để tìm folder tên "Workspace"
                for dirpath, dirnames, _ in os.walk(root, topdown=True):
                    # Giới hạn độ sâu quét để tránh lag
                    depth = dirpath.count(os.path.sep) - root.count(os.path.sep)
                    if depth > 2: 
                        del dirnames[:] # Dừng quét sâu hơn
                        continue
    ...
    # Lọc những đường dẫn tồn tại thực sự
    valid_paths = [p for p in possible_paths if os.path.exists(p) and os.path.isdir(p)]
    
    print(f"\033[1;36m[ System ] Found {len(valid_paths)} Workspace folders.\033[0m")
    return valid_paths

# Khởi tạo workspace paths
...
class Runner:
    ...
    TELEPORT_GRACE = 180   # 3 phút
    TIMEOUT_LIMIT = 15     # 15 giây không cập nhật file -> Chết (Tăng lên vì I/O chậm hơn HTTP)
    
    launch_times = {}      # Lưu thời gian launch
    proc_cache = {}        # Cache process CPU
    path_cache = {}        # Cache đường dẫn file heartbeat (UID -> Path)

    @classmethod
    def get_package_cpu(cls, package_name):
        try:
            proc = cls.proc_cache.get(package_name)
            if proc and proc.is_running(): return proc.cpu_percent(interval=0.1)
    ...
    @classmethod
    def get_heartbeat_data(cls, user_id):
        """Tìm và đọc file heartbeat của user"""
        filename = f"heartbeat_{user_id}.txt"
        
        # 1. Thử đọc từ đường dẫn đã cache
        if user_id in cls.path_cache:
            path = cls.path_cache[user_id]
            if os.path.exists(path):
                return cls._read_file(path)
            else:
                del cls.path_cache[user_id] # File bị xóa/dời -> Reset cache

        # 2. Quét toàn bộ danh sách workspace
        workspace_list = globals().get("workspace_paths", [])
        for ws_path in workspace_list:
            full_path = os.path.join(ws_path, filename)
            if os.path.exists(full_path):
                cls.path_cache[user_id] = full_path # Lưu cache
                return cls._read_file(full_path)
        ...
        try:
            with open(path, "r", encoding="utf-8", errors="ignore") as f:
                ...
                if len(parts) >= 2:
                    return {
                        "status": parts[0],
                        "timestamp": int(float(parts[1])), # Xử lý cả float nếu có
                        "path": path
                    }
        except: pass
        return None
    ...
    def launch_package_sequentially(cls, server_links):
        # 1. Tự động cài Script Lua (Nếu bật config)
        if globals().get("check_exec_enable") == "1":
            print("\033[1;36m[ Executor ] Checking and installing Heartbeat Script...\033[0m")
            detected = ExecutorManager.detect_executors()
            if detected:
                # Đảm bảo cài script mới nhất từ file checkui.lua
                if os.path.exists("Shouko.dev/checkui.lua"):
                    ExecutorManager.write_lua_script(detected)
                else:
                    print("\033[1;33m[ Executor ] Script file not found. Re-run Menu 5 to fix.\033[0m")
            else:
                print("\033[1;33m[ Executor ] No executor folder found (Check storage permission).\033[0m")

        # 2. Vòng lặp Launch từng acc
        for package_name, server_link in server_links:
            user_id = globals()["_user_"].get(package_name, "Unknown")
            if user_id == "Unknown": continue
            
            # Reset cache cũ để tránh đọc nhầm dữ liệu của lần chạy trước
            if package_name in cls.proc_cache: del cls.proc_cache[package_name]
            if user_id in cls.path_cache: del cls.path_cache[user_id]
            
            # Xóa file heartbeat cũ trong workspace (Dọn rác)
            for ws_path in globals().get("workspace_paths", []):
                try:
                    old_file = os.path.join(ws_path, f"heartbeat_{user_id}.txt")
                    if os.path.exists(old_file): os.remove(old_file)
                except: pass

            # Set Status ảo "Preparing"
            with status_lock:
                globals()["package_statuses"][package_name] = {
                    "Username": FileManager.get_username(user_id),
                    "Status": "\033[1;33mPreparing Launch...\033[0m"
                }

            print(f"\033[1;32m[ Shouko.dev ] - Launching: {package_name}\033[0m")
            
            # Gọi lệnh mở Roblox
            RobloxManager.launch_roblox(package_name, server_link)
            
            # Ghi nhận thời gian bắt đầu chạy
            cls.launch_times[user_id] = time.time()
            
            # Cập nhật trạng thái Booting
            with status_lock:
                globals()["package_statuses"][package_name]["Status"] = "\033[1;33mBooting (Grace Period 5m)...\033[0m"
            
            # Nghỉ 15s để máy đỡ lag
            time.sleep(15)
            

    @classmethod
    def monitor_presence(cls, server_links, stop_event):
        print("\033[1;36m[ Runner ] - File-Based Monitor Active...\033[0m")
        low_cpu_start = {}
        active_users = set()
        ...
        while not stop_event.is_set():
            try:
                ...
                for pkg, _ in server_links:
                    ...
                    # --- ĐỌC FILE ---
                    hb_data = cls.get_heartbeat_data(uid)
                    ...
                    cpu_tag = f" | CPU: {pkg_cpu:.1f}%"

                    # === TRƯỜNG HỢP 1: CHƯA CÓ FILE ===
                    if not hb_data:
                        if uid in active_users: active_users.remove(uid)
                        lt = cls.launch_times.get(uid, 0)
                        
                        if now - lt < cls.BOOT_GRACE:
                            rem = int(cls.BOOT_GRACE - (now - lt))
                            st = f"\033[1;33mBooting ({rem}s){cpu_tag}\033[0m"
                            
                            # Check Low CPU
                            if (now - lt > 20) and pkg_cpu < 20.0:
                                if uid not in low_cpu_start: low_cpu_start[uid] = now
                                elif now - low_cpu_start[uid] > 15:
                                    st = f"\033[1;31mFROZEN ({pkg_cpu:.1f}% > 15s)\033[0m"; rejoin = True
                            else:
                                if uid in low_cpu_start: del low_cpu_start[uid]
                        else:
                            st = "\033[1;31mNO SIGNAL (File Not Found)\033[0m"; rejoin = True
                    
                    # === TRƯỜNG HỢP 2: CÓ FILE ===
                    else:
                        if uid not in active_users: 
                            active_users.add(uid)
                            if uid in low_cpu_start: del low_cpu_start[uid]
                        
                        last_ts = hb_data["timestamp"]
                        status = hb_data["status"]
                        diff = now - last_ts
                        
                        if status == "TELEPORT":
                            if diff < cls.TELEPORT_GRACE: 
                                st = f"\033[1;35mTeleporting ({int(diff)}s){cpu_tag}\033[0m"
                            else: 
                                st = "\033[1;31mStuck Teleport\033[0m"; rejoin = True
                        
                        elif status == "TELEPORT_FAIL":
                            # Script báo fail -> Chuyển về Alive nhưng cảnh báo
                            st = f"\033[1;33mTeleport Failed (Retrying...){cpu_tag}\033[0m"
                            # Không rejoin, để script tự retry hoặc đợi Alive tiếp theo
                        
                        elif status == "SHUTDOWN":
                            st = "\033[1;31mGame Closed\033[0m"; rejoin = True
                        
                        else: # ALIVE
                            if diff > cls.TIMEOUT_LIMIT:
                                lt_launch = cls.launch_times.get(uid, 0)
                                if now - lt_launch < cls.BOOT_GRACE: 
                                    st = f"\033[1;33mLag/Booting{cpu_tag}\033[0m"
                                else: 
                                    st = f"\033[1;31mCRASHED (Last: {int(diff)}s ago)\033[0m"; rejoin = True
                            else:
                                # Hiển thị độ trễ file (thường 0-2s)
                                color = "\033[1;32m" if diff < 3 else "\033[1;33m"
                                st = f"{color}Alive (Delay: {diff:.1f}s){cpu_tag}\033[0m"
                    ...
                    if rejoin:
                        # Xóa file cũ để tránh vòng lặp đọc lại file cũ
                        if hb_data and "path" in hb_data:
                            try: os.remove(hb_data["path"])
                            except: pass
...
def main():
    ...
    # --- SCRIPT LUA MỚI (V5 - FILE BASED) ---
    LUA_NEW = r"""local Plr=game:GetService("Players").LocalPlayer;if not Plr then return end;local UID=tostring(Plr.UserId);local FN="heartbeat_"..UID..".txt";local TS=game:GetService("TeleportService");local function up(s)pcall(function()if writefile then writefile(FN,s.."|"..tostring(os.time()))end end)end;TS.TeleportInitFailed:Connect(function()up("TELEPORT_FAIL");task.wait(1);up("ALIVE")end);Plr.OnTeleport:Connect(function()up("TELEPORT")end);game:BindToClose(function()up("SHUTDOWN");task.wait(0.5)end);task.spawn(function()up("ALIVE");while true do task.wait(2);up("ALIVE")end end)"""
    ...
    while True:
        ...
        if setup_type == "2":
            ...
            try:
                # Script V5 (File Based)
                LUA_SRC = r"""local Plr=game:GetService("Players").LocalPlayer;if not Plr then return end;local UID=tostring(Plr.UserId);local FN="heartbeat_"..UID..".txt";local TS=game:GetService("TeleportService");local function up(s)pcall(function()if writefile then writefile(FN,s.."|"..tostring(os.time()))end end)end;TS.TeleportInitFailed:Connect(function()up("TELEPORT_FAIL");task.wait(1);up("ALIVE")end);Plr.OnTeleport:Connect(function()up("TELEPORT")end);game:BindToClose(function()up("SHUTDOWN");task.wait(0.5)end);task.spawn(function()up("ALIVE");while true do task.wait(2);up("ALIVE")end end)"""
                ...
                if globals()["lua_script_template"]:
                    ...
                    print(f"\033[1;32mScript updated: {cfg_path}\033[0m")
                else:
