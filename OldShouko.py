...
class Runner:
    BOOT_GRACE = 300
    HEARTBEAT_TIMEOUT = 15 # Timeout bình thường
    TELEPORT_MAX_WAIT = 60 # Timeout khi đang Teleport
    ...
    def monitor_presence(cls, server_links, stop_event):
        print("\033[1;36m[ Runner ] - Monitor V11 (Strict + Teleport Grace 60s)...\033[0m")
            ...
            try:
                ...
                for pkg, _ in server_links:
                    ...
                    if now - last_hb < cls.HEARTBEAT_TIMEOUT and last_hb > 0:
                        ...
                        if status_lua == "TELEPORT":
                            # Bắt đầu tính giờ Teleport
                            ...
                            # Nếu quá 60s mà vẫn Teleport -> Kẹt -> Rejoin
                            if elapsed > cls.TELEPORT_MAX_WAIT:
                                st = "\033[1;31mTeleport Stuck (>60s)\033[0m"
                                rejoin = True

                        elif status_lua == "TELEPORT_FAIL":
                            # Fail -> Reset trạng thái Teleport -> Về Alive
                            if uid in cls.teleport_start: del cls.teleport_start[uid]
                            st = f"\033[1;33mTeleport Failed -> Retrying... | CPU: {pkg_cpu:.1f}%\033[0m"
                        
                        else: # ALIVE
                            # Alive -> Reset trạng thái Teleport -> OK
                            if uid in cls.teleport_start: del cls.teleport_start[uid]
                            st = f"\033[1;32mConnected (Ping: {now-last_hb:.1f}s) | CPU: {pkg_cpu:.1f}%\033[0m"

                    # 2. Mất Heartbeat
                    else:
                        # Đang trong trạng thái chờ Teleport (nhưng script tạm ngưng gửi do chuyển server)
                        if uid in cls.teleport_start:
                            elapsed = int(now - cls.teleport_start[uid])
                            if elapsed < cls.TELEPORT_MAX_WAIT:
                                st = f"\033[1;35mTeleporting (No Signal)... ({elapsed}s/60s)\033[0m"
                            else:
                        ...
                        # Mất kết nối thật sự
                            ...
                            st = "\033[1;31mNO SIGNAL (Timeout 15s)\033[0m"
                            rejoin = True
...
def main():
    ...
    LUA_NEW = r"""local Plr=game:GetService("Players").LocalPlayer;local TS=game:GetService("TeleportService");local FILE="heartbeat_"..tostring(Plr.UserId)..".txt";local function log(s)pcall(function()if writefile then writefile(FILE,s.."|"..tostring(os.time()))end end)end;TS.TeleportInitFailed:Connect(function()log("TELEPORT_FAIL");task.wait(1);log("ALIVE")end);Plr.OnTeleport:Connect(function()log("TELEPORT")end);task.spawn(function()log("ALIVE");while task.wait(2)do log("ALIVE")end end)"""
    ...
    while True:
        ...
        if setup_type == "2":
            ...
            try:
                # Script Lua V9 (Workspace Mode - Detect Fail)
