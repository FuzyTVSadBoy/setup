...
class Runner:
    ...
    def monitor_presence(cls, server_links, stop_event):
        print("\033[1;36m[ Runner ] - Strict Monitor V14 (Script Trust + Low CPU Fix)...\033[0m")
        low_cpu_start = {}
        ...
        while not stop_event.is_set():
            try:
                ...
                for pkg, _ in server_links:
                    ...
                    # 1. LOW CPU CHECK (Chống Treo Logo/Crash Ngầm)
                    if (now - lt_launch > 20) and (pkg_cpu < 20.0):
                        if uid not in low_cpu_start:
                            low_cpu_start[uid] = now
                        elif now - low_cpu_start[uid] > 12:
                            st = f"\033[1;31mFROZEN/CRASH ({pkg_cpu:.1f}% > 12s)\033[0m"
                            rejoin = True
                    else:
