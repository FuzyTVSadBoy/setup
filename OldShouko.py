...
# --- CẤU HÌNH HEARTBEAT SERVER ---
# Biến lưu dữ liệu tín hiệu từ Lua Script gửi về
raw_heartbeat_data = {}
heartbeat_lock = threading.Lock()

class HeartbeatHandler(http.server.BaseHTTPRequestHandler):
    """Xử lý tín hiệu POST từ script Lua gửi về localhost:5000"""
    def do_POST(self):
        if self.path == '/heartbeat':
            try:
                ...
                if user_id:
                    with heartbeat_lock:
                        # Lưu thời gian nhận được tín hiệu theo giờ của máy chạy tool
                        data["server_received_time"] = time.time()
                        raw_heartbeat_data[user_id] = data
    ...
    # Tắt log in ra màn hình để đỡ rối console
    def log_message(self, format, *args):
        return

def start_heartbeat_server():
    """Hàm chạy server ngầm"""
    try:
        # Cho phép reuse port để tránh lỗi khi restart tool nhanh
        socketserver.TCPServer.allow_reuse_address = True
        server = socketserver.TCPServer(('127.0.0.1', 5000), HeartbeatHandler)
        t = threading.Thread(target=server.serve_forever, daemon=True)
        t.start()
        # print("\033[1;32m[Heartbeat] Server listening on port 5000\033[0m")
    except Exception as e:
        print(f"\033[1;31m[Heartbeat] Lỗi khởi động server: {e}\033[0m")
        
class Runner:
    def __init__(self, roblox_manager, file_manager, account_monitor=None):
        # account_monitor là tham số thừa để tương thích nếu gọi sai, có thể bỏ qua
        self.roblox_manager = roblox_manager
        self.file_manager = file_manager
        
        # Cấu hình thời gian (Giây)
        self.BOOT_GRACE = 300       # 5 phút đầu không rejoin
        self.TELEPORT_GRACE = 180   # 3 phút chờ khi teleport
        self.TIMEOUT_LIMIT = 5      # Quá 5s không có heartbeat -> Chết
        self.CPU_THRESHOLD = 95     # CPU > 95% -> Tạm dừng logic rejoin
        
        # Lưu thời điểm launch để tính Boot Grace
        self.launch_times = {}

    def launch_package_sequentially(self, server_links):
        # Khởi động server lắng nghe ngay khi bắt đầu
        start_heartbeat_server()
        
        # Logic launch cũ, giữ nguyên nhưng thêm ghi nhận thời gian
        for package_name, server_link in server_links:
            ...
            self.roblox_manager.launch_roblox(package_name, server_link)
            
            # Ghi nhớ thời điểm launch
            self.launch_times[user_id] = time.time()
            
            # Tạo trạng thái giả ban đầu cho UI
            with status_lock:
                globals()["package_statuses"][package_name] = {
                    "Username": FileManager.get_username(user_id),
                    "Status": "\033[1;33mBooting (Grace Period)...\033[0m"
                }
            
            time.sleep(15) # Delay 15s giữa các acc

    def monitor_presence(self, server_links, stop_event):
        """Logic theo dõi mới: Dựa vào Heartbeat thay vì check file/API"""
        print("\033[1;36m[ Runner ] - Bắt đầu theo dõi Heartbeat...\033[0m")
        
        while not stop_event.is_set():
            try:
                # 1. Đo CPU để tránh lag máy
                if psutil.cpu_percent(interval=None) > self.CPU_THRESHOLD:
                    time.sleep(3)
                    continue

                current_time = time.time()
                
                # Duyệt qua danh sách các package đang chạy
                for package_name, _ in server_links:
                    user_id = globals()["_user_"].get(package_name)
                    if not user_id: continue

                    # Lấy dữ liệu heartbeat mới nhất
                    hb_data = {}
                    with heartbeat_lock:
                        if user_id in raw_heartbeat_data:
                            hb_data = raw_heartbeat_data[user_id]

                    # --- XỬ LÝ TRẠNG THÁI ---
                    status_ui = "\033[1;30mUnknown\033[0m"
                    should_rejoin = False
                    
                    # Nếu chưa có heartbeat nào
                    if not hb_data:
                        launch_t = self.launch_times.get(user_id, 0)
                        if current_time - launch_t < self.BOOT_GRACE:
                            remain = int(self.BOOT_GRACE - (current_time - launch_t))
                            status_ui = f"\033[1;33mBooting ({remain}s)\033[0m"
                        else:
                            status_ui = "\033[1;31mNO SIGNAL (Dead)\033[0m"
                            should_rejoin = True
                    else:
                        # Đã có heartbeat -> Kiểm tra độ trễ
                        last_sv_time = hb_data.get("server_received_time", 0)
                        raw_status = hb_data.get("status", "ALIVE")
                        diff = current_time - last_sv_time
                        
                        if raw_status == "TELEPORT":
                            if diff < self.TELEPORT_GRACE:
                                status_ui = f"\033[1;35mTeleporting ({int(diff)}s)\033[0m"
                            else:
                                status_ui = "\033[1;31mStuck Teleport\033[0m"
                                should_rejoin = True
                        
                        elif raw_status == "SHUTDOWN":
                            status_ui = "\033[1;31mGame Closed/Kick\033[0m"
                            should_rejoin = True
                            
                        else: # Trạng thái ALIVE
                            if diff > self.TIMEOUT_LIMIT:
                                # Check lại Boot Grace lần cuối cho chắc
                                launch_t = self.launch_times.get(user_id, 0)
                                if current_time - launch_t < self.BOOT_GRACE:
                                     status_ui = "\033[1;33mLag/Booting\033[0m"
                                else:
                                    status_ui = f"\033[1;31mCRASHED ({int(diff)}s ago)\033[0m"
                                    should_rejoin = True
                            else:
                                status_ui = "\033[1;32mStable (Alive)\033[0m"

                    # --- CẬP NHẬT UI (Dùng biến global cũ) ---
                    with status_lock:
                        if package_name in globals()["package_statuses"]:
                            globals()["package_statuses"][package_name]["Status"] = status_ui

                    # --- THỰC HIỆN REJOIN ---
                    if should_rejoin:
                        print(f"\033[1;31m[AutoRejoin] Phát hiện {user_id} chết. Đang rejoin...\033[0m")
                        
                        # Xóa dữ liệu cũ để tránh loop
                        with heartbeat_lock:
                            if user_id in raw_heartbeat_data:
                                del raw_heartbeat_data[user_id]
                        
                        # Gọi hàm rejoin có sẵn
                        # Lưu ý: Hàm này cần tìm đúng server link. 
                        # Trong code cũ server link được truyền vào monitor_presence nhưng ở đây ta cần lấy lại.
                        # Ta lấy tạm từ biến server_links truyền vào (loop ở trên)
                        sv_link = dict(server_links).get(package_name) 
                        
                        if sv_link:
                            # Rejoin quy trình chuẩn
                            RobloxManager.kill_roblox_process(package_name)
                            RobloxManager.delete_cache_for_package(package_name)
                            time.sleep(2)
                            ...
                            # Chạy rejoin trong thread mới để không block vòng lặp monitor
                            threading.Thread(target=self.roblox_manager.launch_roblox, args=(package_name, sv_link), daemon=True).start()
                            
                            # Reset thời gian launch
                            self.launch_times[user_id] = time.time()
                            
                            # Ngủ nhẹ 5s để tránh spam lệnh
                            time.sleep(5)
            ...
            time.sleep(2) # Chu kỳ quét

    @staticmethod
    def force_rejoin(server_links, interval, stop_event):
        # Giữ nguyên logic cũ hoặc xóa nếu không cần thiết
        pass

    @staticmethod
    def update_status_table_periodically():
        # Giữ nguyên logic cũ để vẽ bảng
        while True:
            UIManager.update_status_table()
            time.sleep(5) # Tăng lên 5s cho đỡ lag console
