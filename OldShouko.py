...
class ExecutorManager:
    ...
    def write_lua_script(detected_executors):
        ...
        try:
            ...
            console.print(f"[bold red]Error reading script: {e}[/bold red]")
            return
...
class Runner:
    ...
    TIMEOUT_LIMIT = 5      # 5 giây
    
    launch_times = {}      # Lưu thời gian launch
    proc_cache = {}        # Cache process

    @classmethod
    def get_package_cpu(cls, package_name):
        """Đo CPU riêng từng package (Cần ROOT)"""
        try:
            ...
            for p in psutil.process_iter(['name', 'cmdline']):
                try:
                    if package_name in (p.info['cmdline'] or []) or \
                       package_name in (p.info['name'] or ''):
        ...
        if globals().get("check_exec_enable") == "1":
            ...
            if detected:
                if globals().get("lua_script_template"):
                    ...
                    print("\033[1;33m[ Executor ] Chưa có Script Lua (Vào Menu 5 để cài).\033[0m")
            else:
                print("\033[1;33m[ Executor ] Không tìm thấy thư mục Executor.\033[0m")
    ...
    def monitor_presence(cls, server_links, stop_event):
        print("\033[1;36m[ Runner ] - Smart Monitor V3 (Fail Detect + CPU)...\033[0m")
        low_cpu_start = {}
        active_users = set()
        ...
        while not stop_event.is_set():
            try:
                now = time.time()
                scan_interval = 5 if not active_users else 2
                ...
                for pkg, _ in server_links:
                    ...
                    if not hb:
                        ...
                        lt_hb = hb.get("server_received_time", 0)
                        ...
                        if rst == "TELEPORT":
                            ...
                            if diff > cls.TIMEOUT_LIMIT:
                                ...
                                if now - lt_launch < cls.BOOT_GRACE: st = f"\033[1;33mLag/Booting{cpu_tag}\033[0m"
                                else: st = f"\033[1;31mCRASHED ({int(diff)}s)\033[0m"; rejoin = True
                            else: 
                                if diff < 1.5: color = "\033[1;32m"
                                elif diff < 3: color = "\033[1;33m"
                                else: color = "\033[1;31m"
                                st = f"{color}Alive (Ping: {diff:.1f}s){cpu_tag}\033[0m"
...
def main():
    print("\033[1;36m[ System ] Starting Heartbeat Server...\033[0m")
    start_heartbeat_server()
    ...
    if not globals().get("command_8_configured", False):
        ...
        try:
            ...
            with open("Shouko.dev/checkui.lua", "w") as f:
                f.write(LUA_NEW)
            print("\033[1;32m[ Script ] Đã cập nhật Script Lua Heartbeat mới nhất.\033[0m")
        except: pass
